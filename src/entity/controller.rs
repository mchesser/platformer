    ...
    fn update(&mut self, map: &Map, secs: f32) {
        ...
        let x_accel =
            if keyboard.get().is_keydown(keycode::LeftKey) {
                -self.entity.physical_properties().movement_accel * if self.entity.is_on_ground() { 1.0 } else { 0.6 }
            }
        ...
        if self.entity.is_on_ground() && keyboard.get().is_new_keypress(keycode::UpKey) {
            let new_velocity = self.entity.velocity() + Vec2::new(0.0, -self.entity.physical_properties().jump_accel);
